...
class Particle_Propagator():
    ...
    def _analyze_pipe_list(self,pipe_list):
        print("STARTE ANALYZE PIPE LIST __------ KEIN FEHLER AB HIER")
        idx = 0         #running index
    ...
    def _run_jobs(self,jobs, workers=1):
            ...
            try:
                while True:
                    ...
                    for running_idx in range(25): #let each worker run multiple runs at once, 
                        ...
                        queue_worker.put((idx,result, np.array(process)*1000,start),timeout = 15)#send information to output pipe
                        #print(f"w {idx:02d} end {args[1]+running_idx}")
            except Empty:    #just for safety reasons
                print(f"w {idx:02d} finish")
                queue_finish.put((idx))
                return 
            
            print(f"worker {idx:02d}, WHY END UP HERE??-------------------------------")
        
        def information_extractor_worker(queue_finish, worker_queue_list, args):
            finished_pipes_counter = 0
            
            idx = 0         #running index
            position_array = np.zeros((args[0],args[1])).astype("float64")  #array to save movement of the electrons
            result_list = []
            start_list = []
            
            print("START", finished_pipes_counter)
            while finished_pipes_counter < len(worker_queue_list) or idx<args[0]-1:
                print(f"amount finished: {idx}")
                while True:
                    if finished_pipes_counter < len(worker_queue_list):
                        print("AMOUNT FINISHED PIPES ",finished_pipes_counter)

                        try:
                            print("trying receiving")
                            _ = queue_finish.get(block=True, timeout=.1)
                            print("received")
                ...
                for x in range(len(worker_queue_list)):
                    ...
                    while True:
                        try:
                            _,result,process,start = worker_queue_list[x].get(block=True, timeout=.1)
                            #print("start",start)
                            result_list.append(result)
                            start_list.append(start)
            ...
            print("sending info start")   
            queue_finish.put((result_list,position_array,start_list),timeout = 10)  
            print("sending info end")  
            return 
        ...
        p = Process(target=information_extractor_worker, args=[queue_finish, worker_queue_list,
                                                               (len(self.x_list),len(self.rounds_to_save))])
        p.daemon = True
        p.start()
        #processes.append(p)
        p.join()
        ...
        print("trying getting result infos")
        result = queue_finish.get(timeout = 5)
        print("got result infos")
        return result    
